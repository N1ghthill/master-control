...
def build_parser() -> argparse.ArgumentParser:
    ...
    parser.add_argument(
        "--output-dir",
        default=str(REPO_ROOT / "artifacts" / "host-validation"),
...
def main(argv: list[str] | None = None) -> int:
    ...
    baseline_results = _run_baseline_commands() if args.run_baseline else ()
    report = {
...
def _build_settings(*, provider: str, state_dir: Path, db_path: Path) -> Settings:
    from master_control.config import Settings
...
def _validate_slow_host_workflow(app: MasterControlApp) -> dict[str, Any]:
    ...
    except Exception as exc:  # pragma: no cover - validation harness must keep reporting
...
def _validate_failed_service_workflow(app: MasterControlApp) -> dict[str, Any]:
    try:
        ...
        unit_count = (
            failed_services_result["result"].get("unit_count")
            if failed_services_result.get("ok") and isinstance(failed_services_result.get("result"), dict)
            else None
        )
        notes: list[str] = []
        if unit_count == 0:
        ...
        return {
            ...
            "failed_services_tool": {
                ...
                "status": failed_services_result.get("result", {}).get("status")
                if isinstance(failed_services_result.get("result"), dict)
                else None,
            },
            "message_excerpt": _excerpt(payload.get("message")),
            "notes": notes,
...
def _validate_managed_config_workflow(app: MasterControlApp) -> dict[str, Any]:
    try:
        ...
        backup_path = (
            write_result.get("result", {}).get("backup_path")
            if isinstance(write_result.get("result"), dict)
            else None
        )
        ...
        active_recommendations = recommendation_sync["sessions"][0]["recommendations"]["active"]
