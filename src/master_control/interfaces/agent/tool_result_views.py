"""View helpers for planner and renderer summaries over typed tool payloads."""
...
def build_tool_result_view(
    ...
    if tool_name == "top_processes":
        ...
        if status != "ok":
            ...
            return ToolResultView(
                ...
                rendered_summary=(f"Nao foi possivel coletar os processos no momento: {reason}."),
            )
    ...
    if tool_name == "process_to_unit":
        ...
        if status != "ok":
            ...
            return ToolResultView(
                ...
                rendered_summary=(
                    f"Nao foi possivel correlacionar o processo com systemd no momento: {reason}."
                ),
            )
    ...
    if tool_name == "read_journal":
        ...
        if returned_lines is not None:
            summary_updates["logs"] = (
                f"{tracked_unit}: last journal read returned {returned_lines} lines"
            )
    ...
    if tool_name == "read_config_file":
        path = _string_or_default(
            result.get("path"), _argument_string(arguments, "path", "arquivo")
        )
    ...
    if tool_name == "write_config_file":
        ...
        validation_kind = (
            _string_or_none(validation.get("kind")) if isinstance(validation, dict) else None
        )
        planner_summary = (
