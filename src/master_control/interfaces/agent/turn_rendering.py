"""Rendering helpers for interface-owned conversational output."""
...
from master_control.shared.planning import PlanningDecision
    ...
    if turn_decision.state == "blocked" and turn_decision.kind == "awaiting_confirmation":
        ...
        if isinstance(pending_execution, dict):
            ...
            if isinstance(approval, dict):
                ...
                if (isinstance(cli_command, str) and cli_command and cli_command in message) or (
...
def render_execution_summary(execution: dict[str, object]) -> str:
    ...
    if not execution.get("ok"):
        if execution.get("pending_confirmation"):
            ...
            if isinstance(approval, dict):
                ...
                return f"{prefix} CLI: `{cli_command}`. Chat: `{chat_command}`."
    ...
    if not isinstance(result, dict):
        return f"`{tool_name}` executou, mas retornou um payload inesperado."
