...
class ProviderAvailabilityTest(unittest.TestCase):
    ...
    def test_probe_ollama_rejects_non_http_base_url(self) -> None:
        with tempfile.TemporaryDirectory() as tmp_dir:
            settings = Settings(
                app_name="master-control",
                log_level="INFO",
            ...
            payload = probe_ollama(settings, binary_lookup=lambda name: None)

        self.assertFalse(payload["available"])
        self.assertIn("http or https", payload["summary"])

    def test_probe_openai_rejects_non_http_base_url(self) -> None:
        with tempfile.TemporaryDirectory() as tmp_dir:
            settings = Settings(
                app_name="master-control",
                log_level="INFO",
