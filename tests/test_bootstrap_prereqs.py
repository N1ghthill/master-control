...
class BootstrapPrereqsTest(unittest.TestCase):
    def test_collect_bootstrap_python_diagnostics_reports_actionable_debian_hint(self) -> None:
        with tempfile.TemporaryDirectory() as tmp_dir:
            ...
            def fake_run(command: list[str], *, capture_output: bool, text: bool, check: bool):
                ...
                if command[1:] == [
                    ...
                    return _CompletedProcess(0, "3.13.5\n", "")
            ...
            with patch("master_control.bootstrap_prereqs.OS_RELEASE_PATH", os_release_path):
                with patch("master_control.bootstrap_prereqs.shutil.which", return_value="/usr/bin/python3"):
                    with patch("master_control.bootstrap_prereqs.subprocess.run", side_effect=fake_run):
        ...
        self.assertEqual(diagnostics["install_hint"], "run: apt install python3.13-venv")
        self.assertIn("python3 3.13.5 found, but stdlib venv is unavailable", diagnostics["summary"])
    ...
    def test_collect_bootstrap_python_diagnostics_reports_ready_state(self) -> None:
        ...
        self.assertEqual(
            diagnostics["summary"],
            "python3 3.13.5 ready for stdlib venv bootstrap.",
        )
